...
public class ExpirableLocalCacheHandler<K, V> implements LocalCacheHandler<K, V> {
    ...
    public ExpirableLocalCacheHandler(
            @NotNull Fetcher<K, V> fetcher, @NotNull ScheduledExecutorService scheduler, long ttl, long cleanupInterval
    ) {
    ...
    public void setScheduler(@NotNull ScheduledExecutorService scheduler) {
        lock.writeLock().lock();
        try {
            this.scheduler = scheduler;
            refreshCleanupTask();
    ...
    private void refreshCleanupTask() {
        ...
        if (cleanupInterval > 0) {
            cleanupTaskFuture = scheduler.scheduleWithFixedDelay(
                    this::cleanupExpired, cleanupInterval, cleanupInterval, TimeUnit.MILLISECONDS
            );
        }
    }
    ...
    private void cleanupExpired() {
        ...
        try {
            // 日志记录。
            LOGGER.info(CacheMessages.message(CacheMessageKey.LOG_EXPIRE_SCAN_STARTED));
            ...
            if (!expiredKeys.isEmpty()) {
                LOGGER.info(
                        CacheMessages.message(CacheMessageKey.LOG_EXPIRED_REMOVED, expiredKeys.size(), expiredKeys)
                );
            } else {
                LOGGER.info(CacheMessages.message(CacheMessageKey.LOG_NO_EXPIRED));
            }
        } finally {
            lock.writeLock().unlock();
        }
