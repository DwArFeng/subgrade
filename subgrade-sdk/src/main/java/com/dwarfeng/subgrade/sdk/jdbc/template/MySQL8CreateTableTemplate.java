 ...
 * @since 1.1.1
 */
public class MySQL8CreateTableTemplate extends GeneralCreateTableTemplate {

    public MySQL8CreateTableTemplate(@NonNull TableDefinition tableDefinition) {
    ...
    protected List<String> internalCreateTableSQL() {
        ...
        ConstraintDefinition pkConstraintDefinition = tableDefinition.getConstraintDefinitions().stream()
                .filter(def -> Objects.equals(def.getType(), PhoenixConstants.TYPE_PRIMARY_KEY))
                .findAny()
    ...
    private String createTableSQL(
        ...
        for (int i = 0; i < columnDefinitions.size(); i++) {
            ...
            // 追加 NULL 或者 NOT NULL
            if (columnCustomDefinition.containsKey(PhoenixConstants.CUSTOM_NULLABLE)) {
                ColumnNullable nullable =
                        (ColumnNullable) columnCustomDefinition.get(PhoenixConstants.CUSTOM_NULLABLE);
            ...
            // 追加 DEFAULT
            if (columnCustomDefinition.containsKey(PhoenixConstants.CUSTOM_DEFAULT)) {
                String defaultValue = (String) columnCustomDefinition.get(PhoenixConstants.CUSTOM_NULLABLE);
        ...
        Set<String> ascColumnNames = (Set<String>) pkConstraintDefinition.getCustomDefinition()
                .getOrDefault(PhoenixConstants.CUSTOM_ASC, Collections.emptySet());
        @SuppressWarnings("unchecked")
        Set<String> descColumnNames = (Set<String>) pkConstraintDefinition.getCustomDefinition()
                .getOrDefault(PhoenixConstants.CUSTOM_DESC, Collections.emptySet());
        String rowTimestampColumnName = (String) pkConstraintDefinition.getCustomDefinition()
                .getOrDefault(PhoenixConstants.CUSTOM_ROW_TIMESTAMP, null);
        // 追加主键序列。
    ...
    private String createIndexSQL(String tableName, IndexDefinition indexDefinition) {
        ...
        sb.append("CREATE");
        // 按需追加 LOCAL
        if (Objects.equals(indexDefinition.getType(), PhoenixConstants.INDEX_TYPE_LOCAL)) {
            sb.append(" LOCAL");
        }
        ...
        List<String> includeColumnNames = (List<String>) customDefinition
                .getOrDefault(PhoenixConstants.CUSTOM_INCLUDE, Collections.emptySet());
    ...
    private void appendOptions(StringBuilder sb, Map<String, Object> customDefinition) {
        ...
        Object val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_SALT_BUCKETS, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("SALT_BUCKETS=%d", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_DISABLE_WAL, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("DISABLE_WAL=%b", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_IMMUTABLE_ROWS, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("IMMUTABLE_ROWS=%b", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_MULTI_TENANT, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("MULTI_TENANT=%b", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_DEFAULT_COLUMN_FAMILY, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("DEFAULT_COLUMN_FAMILY='%s'", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_STORE_NULLS, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("STORE_NULLS=%b", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_TRANSACTIONAL, null);
        if (Objects.nonNull(val)) {
        ...
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_APPEND_ONLY_SCHEMA, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("APPEND_ONLY_SCHEMA=%b", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_AUTO_PARTITION_SEQ, null);
        if (Objects.nonNull(val)) {
            options.add(String.format("AUTO_PARTITION_SEQ='%s'", val));
        }
        val = customDefinition.getOrDefault(PhoenixConstants.CUSTOM_GUIDE_POSTS_WIDTH, null);
        if (Objects.nonNull(val)) {
    ...
    private void appendSplitOn(StringBuilder sb, Map<String, Object> customDefinition) {
        ...
        List<String> splitOnColumns = (List<String>) customDefinition
                .getOrDefault(PhoenixConstants.CUSTOM_SPLIT_POINT, Collections.emptyList());
