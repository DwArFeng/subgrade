...
import com.dwarfeng.subgrade.web.stack.handler.LoginHandler;
...
public class LoginRequiredAdvisor {
    ...
    @Around("@annotation(com.dwarfeng.subgrade.web.sdk.interceptor.login.LoginRequired)")
    public Object around(ProceedingJoinPoint pjp) throws Throwable {
        // 记录日志。
        LOGGER.debug("正在验证是否登录...");

        // 获取 PJP 中的登录 ID 信息。
        String loginId = manager.getLoginId(pjp);
        LOGGER.debug("manager.getLoginId(pjp) = {}", loginId);

        // 判断是否登录。
        if (!loginHandler.isLogin(loginId)) {
            // 没有登录时进行的调度。
            LOGGER.debug("指定的 ID {} 没有登录", loginId);
            return manager.onNotLogin(pjp, loginId);
        }

        // 用户登录后进行的调度。
        LOGGER.debug("指定的 ID {} 登录了", loginId);
        if (isPostpone(pjp)) {
            // 如果注解中注明了要延长登录时间。
            LOGGER.info(WebMessages.message(WebMessageKey.LOG_LOGIN_POSTPONE, loginId));
