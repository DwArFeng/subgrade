...
数据访问层（Dao）封装对持久化数据的读写与查询，是服务层与具体存储技术之间的边界。 在本项目中，Dao 以接口形式定义在
`com.dwarfeng.subgrade.data.stack.dao` 包中，由 `subgrade-data` 模块提供多种实现，
便于按项目技术栈组合选用。
...
所有的 Dao 接口均位于 `com.dwarfeng.subgrade.data.stack.dao` 包中。
...
`com.dwarfeng.subgrade.data.stack.dao.Dao` 是所有数据访问层接口的根，本身不声明方法，作为标记接口表示“属于数据访问层”的类型归属。
...
`com.dwarfeng.subgrade.data.impl.dao` 包提供各技术栈下的实现类。 同类实现的方法语义与接口一致，差异主要体现在底层 API。

实现类通常不声明事务边界，需要在代理类或外层封装中统一加事务，与缓存、服务层文档中的说明一致。

### Hibernate 实现

基于 `subgrade-data` 提供的 `HibernateOperations`，并在当前 `SessionFactory` 的事务上下文中执行操作； 实体/持久化 Bean
之间的转换仍由 `BeanTransformer` 等组件完成。
...
下列通用实现位于 `com.dwarfeng.subgrade.data.impl.service` 包。
...
数据访问层方法抛出 `com.dwarfeng.subgrade.data.stack.exception.DaoException`（通常为包装底层存储或映射异常）。
