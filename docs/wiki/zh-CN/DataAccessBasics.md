...
在本项目中，所有的实体必须统一实现 `com.dwarfeng.subgrade.basic.stack.bean.entity.Entity` 接口，本接口签名如下：
...
其中，`K` 是主键的类型，必须实现 `com.dwarfeng.subgrade.basic.stack.bean.key.Key` 接口。
...
本项目中所有的内置主键均在 `com.dwarfeng.subgrade.basic.stack.bean.key` 包中定义。
...
所有的数据访问层接口都必须继承自 `com.dwarfeng.subgrade.data.stack.dao.Dao` 接口，该接口本身是一个标记接口，
表示该接口是一个数据访问层接口。
...
所有的缓存都必须继承自 `com.dwarfeng.subgrade.data.stack.cache.Cache` 接口，该接口本身是一个标记接口，
表示该接口是一个数据缓存接口。
...
按照本项目的核心思想，所有对外暴露的服务都应该实现 `com.dwarfeng.subgrade.basic.stack.service.Service` 接口，
数据访问机制也不例外。
