...
/**
 * 使用基于 Hibernate 并且拥有本地 SQL 加速查询能力的 PresetLookupDao。
 *
 * <p>
 * 该类拥有 {@link HibernateAcceleratePresetLookupDao#nativeSqlLookup} 属性，该属性应当实现所有预设的本地 SQL 查询功能，
 * 并且在理论上讲，本地查询的效率不应低于 Hibernate 框架的 Criteria 查询；同时，该类拥有
 * {@link HibernateAcceleratePresetLookupDao#presetCriteriaMaker} 属性，该属性作为兜底，当本地 SQL 查询发生错误时，
 * 使用该属性的预设查询方法进行兜底性质的查询，虽然效率比本地 SQL 查询慢，但不至于出错。<br>
 * 当一个预设查询被调用时，nativeSqlLookup 的查询方法被首先调用，数据访问层尝试使用本地 SQL 查询直接返回结果，
 ...
 *
 * <p>
 * 该类在实际应用时，如果目标数据库是可变的，可能存在针对某一种或几种数据库有本地 SQL 加速查询，
 * 而另外一种或几种数据库没有本地 SQL 加速查询的功能。这时候就需要在实例化 {@link PresetLookupDao} 时进行额外的判断，
 * 如果目标数据库支持本地 SQL 加速查询，则生成 {@link HibernateAcceleratePresetLookupDao}，否则，
 * 生成 {@link HibernatePresetLookupDao}。<br>
 * 实现 {@link com.dwarfeng.subgrade.sdk.hibernate.nativesql.DialectNativeSqlLookup}
 * 接口便可方便地判断一个本地 SQL 查询是否支持指定的数据库（方言）。<br>
 * 利用 {@link HibernateDaoFactory#newPresetLookupDao(HibernateTemplate, BeanTransformer, Class, PresetCriteriaMaker, NativeSqlLookup)}
 ...
 * 可以便捷地实现目标数据库可变时的本地 SQL 加速查询判断逻辑。
 *
 * @author DwArFeng
 * @see HibernateDaoFactory#newPresetLookupDao(HibernateTemplate, BeanTransformer, Class, PresetCriteriaMaker, NativeSqlLookup)
 ...
 * @since 1.2.8
 */
public class HibernateAcceleratePresetLookupDao<E extends Entity<?>, PE extends Bean & Entity<?>> implements
        PresetLookupDao<E> {
    ...
    private NativeSqlLookup<PE> nativeSqlLookup;
    ...
    public List<E> lookup(String preset, Object[] objs) throws DaoException {
        try {
            try {
                List<PE> nativeLookup = template.executeWithNativeSession(
                        session -> session.doReturningWork(
                                connection -> nativeSqlLookup.lookupEntity(connection, preset, objs)
                        )
                );
                assert nativeLookup != null;
                return nativeLookup.stream()
                        .map(entityBeanTransformer::reverseTransform).collect(Collectors.toList());
            } catch (Exception e) {
                LOGGER.warn(
                        "使用本地 SQL 执行预设查询时发生异常，本次将使用 Criteria 进行查询。预设: " + preset +
    ...
    public List<E> lookup(String preset, Object[] objs, PagingInfo pagingInfo) throws DaoException {
        try {
            try {
                List<PE> nativeLookup = template.executeWithNativeSession(
                        session -> session.doReturningWork(
                                connection -> nativeSqlLookup.lookupEntity(connection, preset, objs, pagingInfo)
                        )
                );
                assert nativeLookup != null;
                return nativeLookup.stream()
    ...
    public int lookupCount(String preset, Object[] objs) throws DaoException {
        try {
            try {
                Integer result = template.executeWithNativeSession(
                        session -> session.doReturningWork(
                                connection -> nativeSqlLookup.lookupCount(connection, preset, objs)
                        )
                );
                assert result != null;
                return result;
    ...
    public NativeSqlLookup<PE> getNativeSqlLookup() {
        return nativeSqlLookup;
    }

    public void setNativeSqlLookup(@NonNull NativeSqlLookup<PE> nativeSqlLookup) {
        this.nativeSqlLookup = nativeSqlLookup;
    }
}
