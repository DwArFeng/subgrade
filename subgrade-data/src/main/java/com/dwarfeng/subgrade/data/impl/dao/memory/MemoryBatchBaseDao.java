...
import com.dwarfeng.subgrade.data.sdk.memory.io.MapResourceBridge;
...
public class MemoryBatchBaseDao<K extends Key, E extends Entity<K>> implements BatchBaseDao<K, E> {
    ...
    public MemoryBatchBaseDao(@NotNull Map<K, E> memory) {
        this.memory = memory;
    }
    ...
    public void batchUpdate(List<E> elements) throws DaoException {
        List<K> collect = elements.stream().map(E::getKey).toList();
        Map<K, E> map = elements.stream().collect(Collectors.toMap(E::getKey, Function.identity()));
     ...
     */
    public void fillData(@NotNull MapResourceBridge<K, E> mrb) throws DaoException {
        try {
     ...
     * @throws DaoException 数据访问层异常。
     */
    public void saveData(@NotNull MapResourceBridge<K, E> mrb) throws DaoException {
        try {
    ...
    public void setMemory(@NotNull Map<K, E> memory) {
        this.memory = memory;
    }
}
