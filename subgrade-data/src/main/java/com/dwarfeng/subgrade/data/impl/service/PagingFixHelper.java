...
import com.dwarfeng.subgrade.basic.stack.exception.PagingException;
...
import com.dwarfeng.subgrade.data.sdk.configuration.SystemPropertyConstants;
    ...
    public static PagingInfo mayFixPagingInfo(PagingInfo pagingInfo) throws PagingException {
        ...
        if (page < 0) {
            ...
            if (LOG_PAGING_WARNING) {
                String message = DataMessages.message(DataMessageKey.PAGING_PAGE_FIXED, page, 0);
                LOGGER.warn(message);
        ...
        if (rows < 0) {
            ...
            // 如果记录分页警告，则记录警告。
            if (LOG_PAGING_WARNING) {
                String message = DataMessages.message(DataMessageKey.PAGING_ROWS_FIXED, rows, Integer.MAX_VALUE);
                LOGGER.warn(message);
                LOGGER.debug(message, new PagingException(pagingInfo));
            }
            // 修正 rows。
